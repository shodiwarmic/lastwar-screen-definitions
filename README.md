...
4. **OCR.** *(Recommended preprocessing: apply the per-channel contrast stretch in [Pre-OCR enhancement](#pre-ocr-enhancement) to the OCR copy first.)* Run word-level text recognition on the cropped image. Engine choice is consumer-local (Cloud Vision, ML Kit, Tesseract, …); the contract operates on `(text, bounding_box)` pairs and is engine-agnostic.
...
9. **Row clustering.** Per `row_clustering.strategy`. `score_anchored` (preferred): each numeric token in the score column ≥ `min_score` is an anchor; collect name tokens in `[anchor.top - up_band_fraction*H, anchor.bottom + down_band_fraction*H]`. A name is **in the band** when its vertical span *overlaps* that range — test overlap, not full containment. Name text is taller than the score digits beside it (rank prefix, capitals, ascenders/descenders), so a containment test drops every same-row name; an Android consumer clustering at line granularity hit exactly this and lost almost all rows. `y_proximity`: group all tokens by vertical proximity then column-extract within each group.
...
*Optional but strongly recommended; configured by `constants.yaml` → `ocr_preprocessing`.*

Before OCR (stage 4), apply **per-channel autocontrast with a 1% cutoff** to a *copy* of the cropped image: for each of R/G/B independently, clip the lightest and darkest 1% of pixels and linearly stretch the surviving range to 0–255. Keep the original (unmodified) image for colour-based tab/window sampling.

Apply per channel — **do not grayscale first.** The top-3 rank rows (gold/silver/bronze highlight) and the user's pinned self-row render coloured text on coloured backgrounds with low *luminance* contrast — e.g. orange text (B≈0) on a salmon background (B≈160). They differ mostly in the **blue channel**, so per-channel stretching roughly doubles that separation and the OCR engine reads the score. A grayscale pass collapses exactly that separation and erases the text instead (an Android consumer that grayscaled regressed from ~96/97 to ~21/97 rows on a Kills screen).

Reference implementations:
- Python / Cloud Vision: `lastwar-ocr-service` `app/utils/image_utils.py` → `enhance_for_ocr` = `PIL.ImageOps.autocontrast(img, cutoff=1)`.
- Kotlin / ML Kit: `lastwar-android-scanner` `ScreenCaptureService.enhanceForOcr` (per-channel histogram LUT, equivalent to the above).
